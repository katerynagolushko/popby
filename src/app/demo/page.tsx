...
/** Full interactive demo — real London map, no Supabase. */
export default function DemoPage() {
  const [people, setPeople] = useState<DemoPerson[]>(INITIAL_DEMO_PEOPLE);
  ...
  function handleGoLive(payload: GoLivePayload) {
    ...
    setMyLive(session);
    setShowGoLive(false);
    showToast("You're live on the map!");
  }
  ...
  function handleConnect(userId: string) {
    ...
    setTimeout(() => {
      ...
      showToast("They accepted — you can message now (demo)");
    }, 1500);
  }
    ...
    <div className="h-[100dvh] flex flex-col relative bg-paper overflow-hidden">
      ...
      {selected && (
        <PersonSheet
          ...
          onMessage={() => showToast("Messaging opens after sign-in — demo ends here")}
      ...
      {showRating && selected && (
        <RatingModal
          ...
          onSubmitted={() => {
            setShowRating(false);
            showToast("Rating saved (demo — sign in for real)");
          }}
        />
      )}
      ...
      {mapPeople.length > 0 && !myLive && (
        <div className="absolute top-14 inset-x-4 z-[500] pointer-events-none">
          <p className="text-center text-xs text-muted bg-white/90 backdrop-blur rounded-xl px-4 py-2 shadow border border-paper-3">
            {people.length} people live near Old Street — tap a pin or go live
          </p>
        </div>
      )}
    </div>
