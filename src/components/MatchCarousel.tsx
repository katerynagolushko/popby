...
interface MatchCarouselProps {
  ...
  /** Demo: Anyone vs Friends only filter. */
  friendsOnly?: boolean;
  emptyCopy?: string;
    ...
    >
        ...
        <div className="mt-3 px-0.5">
          ...
          <p className="text-base sm:text-lg text-navy/70 mt-1.5 leading-snug">
            {friendsOnly
              ...
              : "Connect. If they connect back, you&apos;re a pair. Swipe sideways. Tap a card for more."}
          </p>
        </div>
      </header>
      ...
      {count === 0 ? (
        <div className="flex-1 min-h-0 flex items-center justify-center px-6">
          <p className="text-lg sm:text-xl text-navy/80 text-center leading-snug max-w-sm">
            {emptyCopy ?? "No one free right now."}
          </p>
        </div>
      ) : (
        <div
        ...
        >
          ...
          >
            {matches.map((p, i) => {
              ...
              const friend = isFriend?.(p.profile.id) ?? false;
                  ...
                  >
                    <div className="relative flex-[2.2] min-h-[52%] bg-paper-2 overflow-hidden">
                      ...
                      <div className="absolute top-2.5 right-2.5 flex flex-col items-end gap-1">
                        {friend && (
                          <span className="bg-accent text-white text-sm font-bold px-2 py-1 rounded-lg">
                            Friend
                          </span>
                        )}
                        {status === "accepted" && !friend && (
                          <span className="bg-mint text-navy text-sm font-bold px-2 py-1 rounded-lg">
                            Pair
                          </span>
                        )}
                      </div>
      ...
      <div className="flex-shrink-0 px-4 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-0.5 text-center">
        <p className="text-sm sm:text-base text-navy/70">
          {count === 0
            ? friendsOnly
              ? "Edit → Anyone to see more people"
              : "No matches"
